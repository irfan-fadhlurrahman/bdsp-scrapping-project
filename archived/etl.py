...
from scraper import (
    get_indicator_input_values,
    convert_input_as_index,
    extract,
    INDICATOR_VALUES_FILEPATH,
)
...
def transform(input_dict, result_dict):
    ...
    df = pd.DataFrame(result_dict["content"], columns=result_dict["headers"])
    # Make tabular table
    df2 = df.drop(columns=["No"])
    df2 = df2.melt(
        id_vars=["Indikator", "Satuan"], var_name="Tahun", value_name="Jumlah"
    )
    # Add subsektor, komoditi, provinsi, kabupaten
    columns_to_add = ["subsektor", "komoditas", "prov", "kab"]
    df_info = pd.DataFrame(
        np.eye(df2.shape[0], len(columns_to_add)), columns=columns_to_add
    )
...
def main(input_dict):
    ...
    output_path = (
        f"data/DataKeluaranIndikator_{komoditas}_{level}_{tahun_awal}_{tahun_akhir}.csv"
    )
